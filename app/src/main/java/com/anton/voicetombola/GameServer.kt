...
class GameServer(private val port: Int = 8889) {
    ...
    private fun processRequest(request: String): String {
        return try {
            ...
            // Gestione limite giocatori (max 6)
            synchronized(this) {
                if (clientId.isNotEmpty()) {
                    if (!playerSessions.containsKey(clientId)) {
                        ...
                        Log.d(
                            "GameServer",
                            "New player connected: $clientId -> Player #$nextPlayerNum (Total: ${playerSessions.size})"
                        )
                    }
                }
            }
            ...
            when (action) {
                ...
                "claimCard" -> {
                    ...
                    synchronized(this) {
                        ...
                        // Se la carta è già di un altro, errore
                        ...
                        // Assegna in modo atomico
                        if (clientId.isNotEmpty()) {
                            cardOwners[cardId] = clientId
                        }
                        occupiedCards.add(cardId)
